#!/usr/bin/env python3
"""
Script para copiar ServiceMonitor.js e ServiceMonitor.css para o Linux.
Execute no servidor Linux:
  python3 _copy_to_linux.py
"""

JS_CONTENT = r"""import React, { useState, useEffect, useCallback } from 'react';
...
.sm-tab:hover { border-color: var(--accent, #3b82f6); color: var(--text-primary, #e2e8f0); }
.sm-tab.active { background: var(--accent, #3b82f6); border-color: var(--accent, #3b82f6); color: #fff; }
...
.sm-stat--ok { border-color: #22c55e33; }
.sm-stat--critical { border-color: #ef444433; }
.sm-stat--info { border-color: #3b82f633; }
...
.sm-stat--ok .sm-stat-value { color: #22c55e; }
.sm-stat--critical .sm-stat-value { color: #ef4444; }
.sm-stat--info .sm-stat-value { font-size: 14px; color: #3b82f6; }
...
.sm-state--running { background: #22c55e22; color: #22c55e; }
.sm-state--stopped { background: #ef444422; color: #ef4444; }
.sm-state--unknown { color: var(--text-muted, #64748b); }
.sm-state--pending {
...
.sm-refresh-btn:hover:not(:disabled) { border-color: var(--accent, #3b82f6); color: var(--text-primary, #e2e8f0); }
.sm-refresh-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
.sm-btn--primary:hover:not(:disabled) { background: #2563eb; }
.sm-btn:disabled, .sm-btn.disabled { opacity: 0.4; cursor: not-allowed; }
...
.sm-row--changed td:first-child::after {
  content: '\\25cf';
  color: #3b82f6;
  font-size: 8px;
...
BASE = '/home/administrador/CorujaMonitor/frontend/src/components'

with open(f'{BASE}/ServiceMonitor.js', 'w', encoding='utf-8') as f:
    f.write(JS_CONTENT)
print('ServiceMonitor.js escrito OK')

with open(f'{BASE}/ServiceMonitor.css', 'w', encoding='utf-8') as f:
    f.write(CSS_CONTENT)
print('ServiceMonitor.css escrito OK')

# Verify
import subprocess
r = subprocess.run(['grep', '-c', 'metric_count', f'{BASE}/ServiceMonitor.js'], capture_output=True, text=True)
print(f'metric_count ocorrencias: {r.stdout.strip()}')
r2 = subprocess.run(['grep', '-c', 'sm-state--pending', f'{BASE}/ServiceMonitor.css'], capture_output=True, text=True)
print(f'sm-state--pending no CSS: {r2.stdout.strip()}')
