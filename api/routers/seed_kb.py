...
def get_kb_entries_full():
    """Retorna 109 entradas completas da base de conhecimento"""
    entries = []
...
@router.post("/populate-full")
async def populate_knowledge_base_full(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_role("admin"))
):
    """Popula a base de conhecimento com 109 itens completos (admin only)"""
    try:
        entries = get_kb_entries_full()

        # Limpar base atual do tenant
        ...
        return {
            "success": True,
            "message": "Base de conhecimento populada com 109 itens",
            "entries_added": count,
            "entries_total": total
        }
