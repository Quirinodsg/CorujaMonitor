"""
Script para criar Base de Conhecimento com 80 itens
...
def create_kb_entries():
    """Gera 80 entradas da base de conhecimento"""
    entries = []
    
    # WINDOWS SERVER (15 itens)
    windows_items = [
        ("win_disk_full", "disk", "critical", "Disco C: Cheio", "Disco sistema >95%", "Temp files", 0.92, ["cleanmgr /sagerun:1"], True, False, "low"),
        ...
        ("win_time_sync_fail", "system", "warning", "Sincronização Tempo Falhou", "Time out of sync", "NTP falhou", 0.87, ["w32tm /resync"], True, False, "low"),
        ("win_cert_expired", "system", "critical", "Certificado Expirado", "SSL cert expired", "Certificado venceu", 0.95, ["Renovar certificado"], False, True, "high"),
        ("win_backup_failed", "system", "critical", "Backup Falhou", "Backup error", "Espaço ou permissão", 0.82, ["Verificar logs backup"], False, True, "high"),
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in windows_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            "solution_description": f"Resolver {title}", "solution_steps": ["Executar comandos"],
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Windows Server"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    
    # LINUX (15 itens)
    linux_items = [
        ("linux_disk_full", "disk", "critical", "Disco Cheio Linux", "Disco >95%", "Logs grandes", 0.90, ["df -h", "du -sh /*"], False, True, "medium"),
        ("linux_mem_high", "memory", "warning", "Memória Alta Linux", "RAM >90%", "Processo alto", 0.83, ["free -m", "top"], False, True, "medium"),
        ("linux_cpu_high", "cpu", "warning", "CPU Alta Linux", "CPU >85%", "Processo alto", 0.78, ["top", "ps aux"], False, True, "medium"),
        ...
        ("linux_ntp_unsync", "system", "warning", "NTP Dessincronizado", "Time drift", "NTP falhou", 0.86, ["ntpdate -u pool.ntp.org"], True, False, "low"),
        ...
        ("linux_load_high", "system", "warning", "Load Average Alto", "Load >5", "Muitos processos", 0.80, ["uptime", "top"], False, True, "medium"),
        ("linux_swap_high", "memory", "warning", "Swap Alto", "Swap >80%", "Pouca RAM", 0.82, ["free -m", "swapon -s"], False, True, "medium"),
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in linux_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Linux"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    
    # DOCKER (10 itens)
    docker_items = [
        ("docker_container_down", "docker", "critical", "Container Parado", "Container stopped", "Container crashed", 0.92, ["docker start <container>"], True, False, "low"),
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in docker_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Linux", "Windows"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in azure_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Azure"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in network_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Network"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in ups_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["UPS"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in ac_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["HVAC"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
    ...
    for sig, stype, sev, title, desc, cause, rate, cmds, auto, approval, risk in webapp_items:
        entries.append({
            "problem_signature": sig, "sensor_type": stype, "severity": sev,
            "problem_title": title, "problem_description": desc,
            "symptoms": [desc], "root_cause": cause, "root_cause_confidence": rate,
            ...
            "solution_commands": cmds, "auto_resolution_enabled": auto,
            "requires_approval": approval, "risk_level": risk,
            "affected_os": ["Web"], "times_matched": 0,
            "times_successful": 0, "success_rate": rate
        })
...
def seed_database():
    ...
    try:
        ...
        print(f"📚 Criando Base de Conhecimento - 80 Itens")
        print(f"🏢 Tenant: {tenant.name}")
        
        existing = db.query(KnowledgeBaseEntry).filter(
            KnowledgeBaseEntry.tenant_id == tenant.id
        ).count()
        print(f"ℹ️  Entradas existentes: {existing}")
        
        entries = create_kb_entries()
        print(f"✨ Gerando {len(entries)} novas entradas...")
        ...
        print(f"\n🎉 Sucesso!")
        print(f"📊 Total de entradas no sistema: {total}")
        print(f"➕ Novas entradas adicionadas: {count}")
...
if __name__ == "__main__":
    print("=" * 70)
    print("🧠 BASE DE CONHECIMENTO - 80 ITENS COMPLETOS")
    print("=" * 70)
    print()
    seed_database()
