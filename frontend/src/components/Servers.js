              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const isNetworkDevice = ['switch', 'router', 'firewall', 'access_point', 'ups'].includes(dt);
                    setNewServer({
                      ...newServer,
                      device_type: dt,
                      monitoring_protocol: isNetworkDevice ? 'snmp' : newServer.monitoring_protocol
                    });
                  }}
                >
              ...
              <button 
                ...
                onClick={() => {
                  if (snmpWizardStep < 3) {
                    ...
                    // Criar sensor SNMP via API
                    (async () => {
                      try {
                        await api.post('/sensors/standalone', {
                          probe_id: parseInt(snmpConfig.probe_id),
                          name: snmpConfig.name,
                          sensor_type: snmpConfig.device_type === 'ap' ? 'snmp_ap' : 'snmp',
                          category: 'snmp',
                          ip_address: snmpConfig.ip_address || null,
                          snmp_version: snmpConfig.snmp_version || 'v2c',
                          snmp_community: snmpConfig.snmp_community || 'public',
                          snmp_port: snmpConfig.snmp_port ? parseInt(snmpConfig.snmp_port) : 161,
                          snmp_oid: snmpConfig.snmp_oid || null,
                          threshold_warning: parseFloat(snmpConfig.threshold_warning) || 80,
                          threshold_critical: parseFloat(snmpConfig.threshold_critical) || 95,
                          description: snmpConfig.description || `Sensor SNMP ${snmpConfig.device_type} - ${snmpConfig.name}`
                        });
                        alert(`✅ Sensor SNMP "${snmpConfig.name}" criado com sucesso na Biblioteca de Sensores!`);
                        setShowSNMPWizard(false);
                        setSNMPWizardStep(1);
                      } catch (error) {
                        console.error('Erro ao criar sensor SNMP:', error);
